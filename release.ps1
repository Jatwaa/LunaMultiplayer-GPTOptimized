...
# GameData/LunaMultiplayer — plugin DLLs
...
# GameData/000_Harmony — Harmony loader
$harmonyDir     = Join-Path $clientStage "GameData\000_Harmony"
$harmonySrc     = Join-Path $repoRoot "External\Dependencies\Harmony\000_Harmony"
...
foreach ($f in @("0Harmony.dll", "HarmonyInstallChecker.dll", "Harmony.version")) {
    ...
    if (Test-Path $src) {
        Copy-Item $src $harmonyDir
    }
}

# Also copy Harmony subdirectory from bin if present (built copy)
$harmonyBinDir = Join-Path $clientBin "Harmony\000_Harmony"
if (Test-Path $harmonyBinDir) {
    foreach ($f in @("0Harmony.dll", "HarmonyInstallChecker.dll", "Harmony.version")) {
        $src = Join-Path $harmonyBinDir $f
        if (Test-Path $src) {
            Copy-Item $src $harmonyDir -Force
        }
    }
}
...
Write-Success "Client ZIP  → LMP-$version-Client.zip"
...
# Exclude runtime-generated dirs and debug symbols
$excludeDirs  = @("logs", "Universe", "Config", "runtimes", "cs", "de", "es", "fr", "it", "ja", "ko", "pl", "pt-BR", "ru", "tr", "zh-Hans", "zh-Hant")
$excludeExts  = @(".pdb")
...
Write-Success "Server ZIP  → LMP-$version-Server.zip"
...
Write-Info "Title   : $title"
Write-Info "Assets  :"
Write-Info "  • LMP-$version-Client.zip  ($('{0:N0}' -f (Get-Item $clientZip).Length) bytes)"
Write-Info "  • LMP-$version-Server.zip  ($('{0:N0}' -f (Get-Item $serverZip).Length) bytes)"
Write-Host ""
...
# Pull release notes from the changelog (everything between first --- and second ---)
$changelogPath = Join-Path $repoRoot "LMP_0.30_2026.md"
$changelogContent = Get-Content $changelogPath -Raw

# Extract the newest entry block from the changelog (between first pair of ---)
$releaseNotes = ""
if ($changelogContent -match '(?s)---\n\n(### \[.*?)---') {
...
$ghResult = gh release create $tag `
    --title $title `
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "  $ghResult" -ForegroundColor Cyan
} else {
    # Tag might already exist — try uploading assets to existing release
    Write-Warn "Release create failed (tag may already exist). Trying to upload assets..."
    gh release upload $tag $clientZip $serverZip --clobber 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Success "Assets uploaded to existing release $tag."
    } else {
        Write-Err "Failed to publish release. Output:"
        Write-Info $ghResult
        Write-Warn "ZIPs are saved at: $staging"
        exit 1
    }
}
