...
namespace LmpClient.VesselUtilities
{
    ...
    public class VesselCommon
    {
        ...
        public static void RemoveVesselFromSystems(Guid vesselId)
        {
            ...
            VesselUpdateSystem.Singleton.RemoveVessel(vesselId);
        }

        /// <summary>
        /// Returns true only when another PLAYER-controlled vessel is in physics range.
        /// Debris, unowned vessels and flags do NOT count — they were previously causing the
        /// active vessel to send at full 50ms rate even when only surrounded by wreckage.
        /// </summary>
        public static bool PlayerVesselsNearby()
        {
            ...
            {
                var v = FlightGlobals.VesselsLoaded[i];
                if (v == null || v == FlightGlobals.ActiveVessel) continue;

                // Only count it as "nearby" if a player holds a control or update lock on it
                if (LockSystem.LockQuery.ControlLockExists(v.id) ||
