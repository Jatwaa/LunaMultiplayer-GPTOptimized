...
namespace LmpClient.Systems.KerbalSys
{
    ...
    public class KerbalSystem : MessageSystem<KerbalSystem, KerbalMessageSender, KerbalMessageHandler>
    {
        ...
        protected override void OnDisabled()
        {
            ...
            GameEvents.onKerbalLevelUp.Remove(KerbalEvents.KerbalLevelUp);
        ...
        {
            // KerbalRoster.SetExperienceTrait (called inside the ProtoCrewMember constructor)
            // crashes with a NullReferenceException if the "trait" value is null or absent.
            // Guard against corrupt/incomplete kerbal data from the server save.
            var traitValue = crewNode.GetValue("trait");
            if (string.IsNullOrEmpty(traitValue))
            {
                var kerbalName = crewNode.GetValue("name") ?? "<unknown>";
                LunaLog.LogWarning($"[LMP]: Kerbal '{kerbalName}' has a missing or empty trait in its ConfigNode — defaulting to Pilot");
                if (traitValue == null)
                    crewNode.AddValue("trait", "Pilot");
                else
                    crewNode.SetValue("trait", "Pilot");
            }
