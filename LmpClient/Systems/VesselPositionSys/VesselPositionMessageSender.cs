...
namespace LmpClient.Systems.VesselPositionSys
{
    public class VesselPositionMessageSender : SubSystem<VesselPositionSystem>, IMessageSender
    {
        ...
        /// <summary>
        /// Returns false if any orbit element, LatLonAlt, or velocity component is
        /// NaN or ±Infinity.  Both cases corrupt the recipient's orbit integration
        /// and must be dropped before transmission.
        /// </summary>
        private static bool OrbitParametersAreOk(Vessel vessel)
        {
            return IsFinite(vessel.orbit.inclination)
                && IsFinite(vessel.orbit.eccentricity)
                && IsFinite(vessel.orbit.semiMajorAxis)
                && IsFinite(vessel.orbit.LAN)
                && IsFinite(vessel.orbit.argumentOfPeriapsis)
                && IsFinite(vessel.orbit.meanAnomalyAtEpoch)
                && IsFinite(vessel.orbit.epoch)
                && IsFinite(vessel.orbit.referenceBody.flightGlobalsIndex)
                // Lat / Lon / Alt
                && IsFinite(vessel.latitude)
                && IsFinite(vessel.longitude)
                && IsFinite(vessel.altitude)
                // Surface-relative velocity
                && IsFinite(vessel.srf_velocity.x)
                && IsFinite(vessel.srf_velocity.y)
                && IsFinite(vessel.srf_velocity.z);
        }

        // True when d is neither NaN nor ±Infinity.
