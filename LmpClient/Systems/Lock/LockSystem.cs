...
namespace LmpClient.Systems.Lock
{
    ...
    public class LockSystem : MessageSystem<LockSystem, LockMessageSender, LockMessageHandler>
    {
        ...
        /// The message is sent BEFORE the local store is modified so that the server
        /// always receives the release request even if processing is interrupted.
        /// </summary>
        private void ReleaseLock(LockDefinition lockDefinition)
        {
            ...
            MessageSender.SendMessage(msgData);   // send first — server must see this
        ...
        /// <summary>
        /// Release the unloaded-update lock on the given vessel
        /// </summary>
        public void ReleaseUnloadedUpdateLock(Guid vesselId)
        {
