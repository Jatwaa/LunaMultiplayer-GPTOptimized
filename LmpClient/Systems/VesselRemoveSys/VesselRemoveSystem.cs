...
namespace LmpClient.Systems.VesselRemoveSys
{
    ...
    public class VesselRemoveSystem : MessageSystem<VesselRemoveSystem, VesselRemoveMessageSender, VesselRemoveMessageHandler>
    {
        ...
        protected override void OnEnabled()
        {
            ...
            SetupRoutine(new RoutineDefinition(30000, RoutineExecution.Update, AutoRemoveOrphanedVessels));
        }
        ...
        /// <summary>
        /// Runs every 30 s.  Kills two categories of vessel the server will never clean up quickly:
        ///
        ///  1. Debris with no lock holder — wreckage nobody owns.  Without this it lingers until
        ///     AutoNuke fires (up to 2 min) and every client in physics range must simulate it.
        ///
        ///  2. PRELAUNCH vessels with no control lock — ghost rockets left on the pad after a
        ///     player launched then disconnected before the server could proto-re-save the vessel.
        ///     These are what caused the launchpad collision bug.
        /// </summary>
        private void AutoRemoveOrphanedVessels()
        {
            if (HighLogic.LoadedScene != GameScenes.FLIGHT &&
                HighLogic.LoadedScene != GameScenes.TRACKSTATION) return;

            // Snapshot to avoid modifying FlightGlobals while iterating
            ...
            foreach (var vessel in vessels)
            {
                ...
                if (VesselWillBeKilled(vessel.id)) continue;

                bool hasAnyLock = LockSystem.LockQuery.ControlLockExists(vessel.id) ||
                                  LockSystem.LockQuery.UpdateLockExists(vessel.id) ||
                                  LockSystem.LockQuery.UnloadedUpdateLockExists(vessel.id);

                // ── Category 1: Orphaned debris ──────────────────────────────────────
                if (vessel.vesselType == VesselType.Debris && !hasAnyLock)
                {
                    LunaLog.Log($"[LMP]: Auto-removing orphaned debris {vessel.id} ({vessel.vesselName}) — no lock holder");
                    KillVessel(vessel.id, true, "Orphaned debris (no lock holder)");
                    continue;
                }

                // ── Category 2: Ghost PRELAUNCH vessel with no control lock ───────────
                if (vessel.situation == Vessel.Situations.PRELAUNCH &&
                    !LockSystem.LockQuery.ControlLockExists(vessel.id))
                {
                    LunaLog.Log($"[LMP]: Auto-removing ghost PRELAUNCH vessel {vessel.id} ({vessel.vesselName}) — no control lock");
                    KillVessel(vessel.id, true, "Ghost PRELAUNCH vessel (no control lock)");
                }
            }
        }
