    ...
    public class WarpSystem : MessageSystem<WarpSystem, WarpMessageSender, WarpMessageHandler>
    {
        ...
        public bool PlayerIsInPastSubspace(string player)
        {
            if (ClientSubspaceList.ContainsKey(player) && CurrentSubspace >= 0)
            {
                ...
                // Guard against race where a subspace is referenced before it's been added
                // to the Subspaces dictionary (direct [] access would throw KeyNotFoundException)
                if (!Subspaces.TryGetValue(playerSubspace, out var playerSubspaceTime))
                    return false;
                if (!Subspaces.TryGetValue(CurrentSubspace, out var currentSubspaceTime))
                    return false;

                return playerSubspace != CurrentSubspace && playerSubspaceTime < currentSubspaceTime;
            }
            return false;
        }
