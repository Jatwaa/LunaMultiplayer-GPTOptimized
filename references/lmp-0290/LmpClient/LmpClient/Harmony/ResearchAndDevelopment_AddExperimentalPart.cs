...
// Type: LmpClient.Harmony.ResearchAndDevelopment_AddExperimentalPart
  ...
  public class ResearchAndDevelopment_AddExperimentalPart
  {
    ...
    private static void PostfixAddExperimentalPart(AvailablePart ap, int __state)
    {
      if (Object.op_Equality((Object) ResearchAndDevelopment.Instance, (Object) null))
        return;
      Dictionary<AvailablePart, int> dictionary = Traverse.Create((object) ResearchAndDevelopment.Instance).Field<Dictionary<AvailablePart, int>>("experimentalPartsStock").Value;
      if (dictionary == null || !dictionary.ContainsKey(ap) || dictionary[ap] == __state)
        return;
      ExperimentalPartEvent.onExperimentalPartAdded.Fire(ap, dictionary[ap]);
    }
  }
}
