...
namespace uhttpsharp.Handlers
{
    public class FileHandler : IHttpRequestHandler
    {
        ...
        public string InstanceHttpRootDirectory { get; set; }
        ...
        public async Task Handle(IHttpContext context, System.Func<Task> next)
        {
            try
            {
                ...
                // Auto-resolve extension-less paths to .html (e.g. "map" -> "map.html")
                if (!Path.HasExtension(requestPath))
                    requestPath += ".html";

                var httpRoot = Path.GetFullPath(InstanceHttpRootDirectory ?? HttpRootDirectory ?? ".");
                if (string.IsNullOrEmpty(httpRoot) || httpRoot == ".")
                    httpRoot = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) ?? ".";

                var path = Path.GetFullPath(Path.Combine(httpRoot, requestPath));

                if (!File.Exists(path))
                {
                    System.Diagnostics.Debug.WriteLine($"FileHandler: File not found: {path}");
                    await next().ConfigureAwait(false);
                    return;
                }

                var rootFull = Path.GetFullPath(httpRoot);
                if (!path.StartsWith(rootFull, StringComparison.OrdinalIgnoreCase))
                {
                    System.Diagnostics.Debug.WriteLine($"FileHandler: Security block - path outside root: {path}");
                    return;
                }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"FileHandler error: {ex.Message}");
            }
        }
    }
}